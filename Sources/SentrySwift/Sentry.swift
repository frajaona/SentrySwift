...
public class Sentry {
    ...
    internal static let sdkName = "SentrySwift"
...
internal extension Sentry {
    ...
    fileprivate func prepareRequest(for event: Event, usingDSN dsn: DSN) throws -> URLRequest {
        let sentryClient = "\(Sentry.sdkName)/\(Sentry.sdkVersion)"
        ...
        let additionalRequestMetadata: [String: Any?] = [
            ...
            "sdk": ["name": Sentry.sdkName, "version": "\(Sentry.sdkVersion)"]
        ]
        ...
        request.httpBody = try requestData.gzipped(level: .defaultCompression)
